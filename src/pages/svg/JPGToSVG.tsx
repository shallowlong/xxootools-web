...
const JPGToSVG = () => {
  ...
  const processFiles = async (files: File[]) => {
    ...
    for (const file of files) {
      // 检查文件类型是否为JPEG
      if (!['image/jpeg', 'image/jpg'].includes(file.type)) continue;
  ...
  return (
    <ToolLayout
      categoryId="svg"
      toolId="jpg-to-svg"
      title={t('categories.svg.tools.jpg-to-svg.name')}
      description={`（${t('categories.svg.tools.jpg-to-svg.description')}）`}
    >
